class BraintreeGateway
  include HTTParty
  include ServiceConstantsConcern
  ...
  def client_token
    ...
    _make_request(operation_name, "mutation { #{operation_name}(input: {}) { clientToken } }")
  end
  ...
  def _make_request(operation_name, query_string, variables_hash = {})
    ...
    return result if is_any_data_present
  ...
  class GraphQLError < StandardError
    ...
    def initialize(graphql_result)
      ...
      LOGGER.error(
        <<~SEMISTRUCTUREDLOG
          "top_level_message" => #{@messages},
          "braintree_request_id" => #{BraintreeGateway.parse_braintree_request_id(graphql_result)},
          "result" => #{graphql_result},
        SEMISTRUCTUREDLOG
      )

      super("GraphQL error occurred") # Add this line to call the parent class constructor
    end
  end
end
